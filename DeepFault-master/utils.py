...
from keras import backend as K # ***important
import sys
...
def calculate_prediction_metrics(Y_test, Y_pred, score):
    ...
    :param Y_pred:
    :param score:
    :return: without any return values
    """
    #Find test and prediction classes
    Y_test_class = np.argmax(Y_test, axis=1)  # argmax 返回最大值的索引， "axis = 1" means horizontal direction
    Y_pred_class = np.argmax(Y_pred, axis=1)

    classifications = np.absolute(Y_test_class - Y_pred_class) # 等价于np.abs()

    correct_classifications = []
    incorrect_classifications = []
    for i in range(1, len(classifications)):  # 为什么从1开始而不是0 ？
    ...
    print(classification_report(Y_test_class, Y_pred_class))    # (precision：所有识别为”1”的数据中，正确的比率是多少；
    print(confusion_matrix(Y_test_class, Y_pred_class))         # recall：所有样本为1的数据中，最后真正识别出1的比率；
    # 混淆矩阵：正确结果都在对角线上，横着看过去不在对角线的就是分类错的。# F1-score: 准确率与召回率的综合,可以认为是平均效果）

    acc = sum([np.argmax(Y_test[i]) == np.argmax(Y_pred[i]) for i in range(len(Y_test))]) / len(Y_test)
    v1 = ceil(acc*10000)/10000 # math.ceil():返回一个大于或等于 x 的的最小整数。
    v2 = ceil(score[1]*10000)/10000 # score[loss, accuracy]
    correct_accuracy_calculation =  v1 == v2  # ？？？问题
    try:
